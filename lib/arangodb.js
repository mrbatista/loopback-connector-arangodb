...
var ArangoDBConnector, Connector, GeoPoint, _, _idIncluded, arangojs, async, debug, generateConnObject, initializeDataSource, merge, qb, url,
...
ArangoDBConnector = (function(superClass) {
  ...
  ArangoDBConnector.prototype.isEdge = function(model) {
    var modelClass;
    modelClass = this.getModelClass(model);
  ...
  ArangoDBConnector.prototype.getCollection = function(model, callback) {
  ...
  ArangoDBConnector.prototype.execute = function(model, command) {
    ...
    return this.notifyObserversAround('execute', context, function(context, done) {
      ...
      if (command === 'query') {
        ...
        return self.db.query(query, bindVars, function(err, result) {
          context.res = result;
          return done(err, result);
        });
      } else {
        return self.getCollection(model, function(err, collection) {
          if (err) {
            return done(err);
          }
          return collection[command](context.req.params[0], function(err, result) {
            context.res = result;
            return done(err, result);
          });
        });
  ...
  ArangoDBConnector.prototype.create = function(model, data, options, callback) {
    ...
    return this.execute(model, 'save', data, function(err, result) {
      ...
      if (condOp) {
        switch (false) {
          ...
          case condOp !== 'like':
            ...
            return aqlArray.push(qb.fn('LIKE')(self.returnVariable + "." + condProp, "" + (assignNewQueryVariable(condValue)), options));
          case condOp !== 'nlike':
            if (options === 'i') {
              options = true;
            } else {
            ...
            return aqlArray.push(qb.not(qb.fn('LIKE')(self.returnVariable + "." + condProp, "" + (assignNewQueryVariable(condValue)), options)));
          case condOp !== 'nin':
  ...
  ArangoDBConnector.prototype.all = function(model, filter, options, callback) {
    ...
    return this.execute(model, 'query', aql, bindVars, function(err, cursor) {
      var cursorToArray;
      if (err) {
        return callback(err);
      }
  ...
  ArangoDBConnector.prototype.update = ArangoDBConnector.prototype.updateAll;
  ...
  ArangoDBConnector.prototype.autoupdate = function(models, cb) {
    ...
    if (this.db) {
      ...
      return async.each(models, (function(model, modelCallback) {
        ...
        ref = self._models[model].properties;
        for (propIdx in ref) {
