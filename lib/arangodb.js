...
ArangoDBConnector = (function(superClass) {
  ...
  ArangoDBConnector.prototype._filter2where = function(model, filter, returnVariable) {
    var aqlArray, assignNewQueryVariable, boundVars, collection, condOp, condProp, condValue, conditionalPart, geoExpr, index, lat, long, ref, ref1;
    if (this.debug) {
      debug("Evaluating where object " + (JSON.stringify(filter)) + " for Model " + model + ": ");
    }
  ...
  ArangoDBConnector.prototype.destroyAll = function(model, where, callback) {
    var aql, bindVars, collVariable, collection, returnVariable, whereFilter;
    if (this.debug) {
    ...
    console.log('query', qb["for"]('@model')["in"]('@@collection'));
    console.log('aql', aql);
    console.log('qb2', qb.obj(aql));
    console.log('raw', qb.expr(aql));
    if (where) {
      whereFilter = this.buildWhere(model, where, returnVariable);
