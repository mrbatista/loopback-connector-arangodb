...
###
class ArangoDB extends Connector
  ###
  ...
  constructor: (settings, dataSource) ->
    Connector.call @, 'arangodb', settings
    
    @debug = settings.debug or debug.enabled
    
    debug('Settings: %j', settings) if @debug
  ...
  ###
  getCollection: (model) =>
    return @getModelClass().settings.arangodb.collection or model
  ...
  transaction: (collections, action, params, callback) =>
    ...
    # TODO: examine if function, make sure there is a params parameter params is not an empty object
    ...
    for condProp, condValue of filter.where
      ...
      if condOp in ['and', 'or']
        ...
        if _.isArray condValue
          ...
          extend true, boundVars, conditionalPart.boundVars
  ...
  _filter2query: (model, filter) =>
    ...
    extend true, boundVars, parts.where.boundVars
  ...
  destroyAll: (model, where, callback) =>
    ...
    if where
      ...
      extend true, bindVars, whereFilter.bindVars
